...
func Load() *Config {
	once.Do(func() {
		cfg = &Config{
			...
			MongoDatabase:  getEnv("MONGODB_DATABASE", "callflowmanager"),
...
func loadEnvFile() error {
	...
	for _, line := range contents {
		...
		for i := 0; i < len(lineStr); i++ {
			if lineStr[i] == '=' {
				os.Setenv(lineStr[:i], lineStr[i+1:])
				break
			}
		}
	}
...
func ConnectDB() (*Database, error) {
	...
	dbOnce.Do(func() {
		...
		db = &Database{Client: client, Database: client.Database(cfg.MongoDatabase)}
	})
	return db, err
}
